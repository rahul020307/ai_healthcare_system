...
function switchTab(tabName) {
  ...
  // Hide all sections
  ['home', 'store', 'maps', 'profile', 'pharmacy'].forEach(tab => {
...
// PHARMACY PORTAL
function renderPharmacyPortal() {
  const tbody = document.getElementById('pharmacy-inventory-body');
  if (!tbody) return;

  tbody.innerHTML = INITIAL_DATA.pharmacyInventory.map(inv => `
    <tr class="hover:bg-slate-900/50">
      <td class="p-3 font-bold text-white">${inv.name}</td>
      <td class="p-3 text-slate-400">${inv.category}</td>
      <td class="p-3 font-bold ${inv.stock < 20 ? 'text-rose-400' : 'text-emerald-400'}">${inv.stock} Units</td>
      <td class="p-3 font-bold text-cyan-300">$${inv.price.toFixed(2)}</td>
      <td class="p-3"><span class="text-[10px] px-2 py-0.5 rounded font-bold ${inv.stock < 20 ? 'bg-rose-500/20 text-rose-300' : 'bg-emerald-500/20 text-emerald-300'}">${inv.status}</span></td>
      <td class="p-3 text-right">
        <button onclick="alert('Restock ${inv.name}')" class="bg-slate-800 text-teal-300 px-2.5 py-1 rounded text-[11px] font-bold">Restock</button>
      </td>
    </tr>
  `).join('');
}

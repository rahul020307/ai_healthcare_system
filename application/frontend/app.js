...
5. **DO NOT DRIVE**: Never attempt to drive yourself to the emergency department. Wait for the medical ambulance crew.`;
  ...
  // 2. URINARY TRACT / HEMATURIA / KIDNEY STONES PROTOCOL
...
• 🚨 **Urgent Warning**: If accompanied by severe flank/back pain, high fever (>102°F) with chills, or inability to pass urine, go to an Emergency Department immediately!`;
  ...
  // 3. GASTROINTESTINAL / VOMITING / DIARRHEA / LOOSE MOTION
...
• **Hydration**: Drink warm water, herbal tea, and clear soups. Rest adequately.
• **Warning**: If fever exceeds 102°F or lasts > 3 days, consult a physician.`;
  }
...
Hello **${memberName}**! Here is your tailored clinical guidance:

• 💊 **Medication**: **Dolo 650mg** or **Combiflam** (Paracetamol + Ibuprofen) — 1 tablet after meals for effective pain and inflammation relief.
• 🩹 **Targeted Clinical Care**: Apply Volini gel or warm compress on affected muscles, rest, and maintain proper posture.
• 🧘 **Rest & Posture**: Rest the affected area, avoid strenuous strain, and keep well-hydrated.
• 🚨 **When to see a Doctor**: If pain is severe or persistent beyond 48 hours, consult a physician.`;
  }

  // 10. Diabetes & Sugar
  if (queryLower.includes('diabet') || queryLower.includes('sugar') || queryLower.includes('glucose') || queryLower.includes('metformin')) {
    return `🤖 **CuraBot AI Clinical Advice: Diabetes & Sugar Control**

Hello **${memberName}**! For blood sugar management:

• **Medication**: **Metformin 500mg** or **Glimepiride 1mg** (as prescribed by your doctor).
• **Diet**: High-fiber, low-glycemic index foods (whole grains, green vegetables). Avoid refined sugar.
• **Tracking**: Log your fasting and post-meal glucose in your **Profile Vitals Tracker**.`;
  }

  // 11. Blood Pressure & Heart Health
  if (queryLower.includes('bp') || queryLower.includes('blood pressure') || queryLower.includes('hypertension')) {
...
• 🩺 **Clinical Overview**: Regarding **"${queryClean}"**, CuraBot AI recommends reviewing active health records, maintaining proper hydration, and monitoring key vitals.

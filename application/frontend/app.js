...
function showDedicatedAuthPage(show = true, mode = 'login') {
  const page = document.getElementById('auth-page');
  if (!page) return;
  page.classList.toggle('hidden', !show);
  if (show) switchAuthPageTab(mode);
}

function switchAuthPageTab(mode) {
  const login = document.getElementById('auth-page-login');
  const reg = document.getElementById('auth-page-register');
  const loginBtn = document.getElementById('btn-auth-page-login');
  const regBtn = document.getElementById('btn-auth-page-reg');
  if (mode === 'register') {
    login?.classList.add('hidden'); reg?.classList.remove('hidden');
    loginBtn?.classList.remove('bg-teal-500','text-slate-950','shadow-md'); loginBtn?.classList.add('text-slate-400');
    regBtn?.classList.add('bg-teal-500','text-slate-950','shadow-md'); regBtn?.classList.remove('text-slate-400');
  } else {
    login?.classList.remove('hidden'); reg?.classList.add('hidden');
    regBtn?.classList.remove('bg-teal-500','text-slate-950','shadow-md'); regBtn?.classList.add('text-slate-400');
    loginBtn?.classList.add('bg-teal-500','text-slate-950','shadow-md'); loginBtn?.classList.remove('text-slate-400');
  }
}

function showDedicatedAuthMessage(text, type = 'error') {
  const el = document.getElementById('auth-page-message');
  if (!el) return;
  if (!text) { el.classList.add('hidden'); el.textContent = ''; return; }
  el.classList.remove('hidden');
  el.classList.remove('bg-rose-500/10','border-rose-500/30','text-rose-300','bg-emerald-500/10','border-emerald-500/30','text-emerald-300','bg-teal-500/10','border-teal-500/30','text-teal-300');
  if (type === 'success') el.classList.add('bg-emerald-500/10','border-emerald-500/30','text-emerald-300');
  else if (type === 'loading') el.classList.add('bg-teal-500/10','border-teal-500/30','text-teal-300');
  else el.classList.add('bg-rose-500/10','border-rose-500/30','text-rose-300');
  el.textContent = text;
...
async function submitDedicatedAuth(mode = 'login') {
  const client = getSupabaseClient();
  if (!client?.auth) {
    showDedicatedAuthMessage('Authentication service is unavailable. Refresh and try again.', 'error');
    return false;
  }

  const email = mode === 'login'
    ? (document.getElementById('auth-page-login-email')?.value || '').trim()
    : (document.getElementById('auth-page-reg-email')?.value || '').trim();
  const password = mode === 'login'
    ? (document.getElementById('auth-page-login-password')?.value || '')
    : (document.getElementById('auth-page-reg-password')?.value || '');

  if (!email || !password || !email.includes('@')) {
    showDedicatedAuthMessage('Enter a valid email address and password.', 'error');
    return false;
  }

  showDedicatedAuthMessage('⏳ Authenticating with CuraAssist…', 'loading');
  ...
  try {
    ...
    if (mode === 'register') {
      ...
      const age = (document.getElementById('auth-page-reg-age')?.value || '30').trim();
      const blood = (document.getElementById('auth-page-reg-blood')?.value || 'O+').trim();
      const city = (document.getElementById('auth-page-reg-city')?.value || 'Hyderabad').trim();
      ({ data, error } = await client.auth.signUp({
        email, password,
        options: { data: { name: name || email.split('@')[0], phone, age, blood, city }, emailRedirectTo: window.location.origin + window.location.pathname }
      }));
    } else {
    ...
    if (error) {
      showDedicatedAuthMessage(error.message || 'Authentication failed.', 'error');
      return false;
    }

    if (!data?.session?.access_token) {
      showDedicatedAuthMessage(mode === 'register' ? 'Account created. Check your email to confirm before signing in.' : 'No active session was returned.', mode === 'register' ? 'success' : 'error');
      return false;
    }

    window.authToken = data.session.access_token;
    showDedicatedAuthMessage('✅ Signed in successfully. Opening CareHub…', 'success');
    await checkSavedSession();
    showDedicatedAuthPage(false);
    document.getElementById('auth-guard-overlay')?.classList.add('hidden');
    return true;
  } catch (err) {
    showDedicatedAuthMessage(err?.message || 'Authentication failed. Please try again.', 'error');
    return false;
  }
}

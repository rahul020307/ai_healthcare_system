...
async function saveProfileEdits() {
  ...
  const preview = document.getElementById('edit-prof-avatar-preview');
  const fallbackAvatar = 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=250';
  let finalAvatar = currentPendingAvatarUrl || preview?.src || fallbackAvatar;
  ...
  try {
    ...
    if (currentPendingAvatarFile) {
      const uploadRes = await fetch(`${API_BASE}/profile/avatar`, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
          data: currentPendingAvatarUrl,
          filename: currentPendingAvatarFile.name
        })
      });
      const uploadData = await uploadRes.json().catch(() => ({}));
      if (!uploadRes.ok) throw new Error(uploadData.detail || 'Avatar upload failed');
      finalAvatar = uploadData?.user?.avatar || uploadData?.user?.avatarUrl;
      if (!finalAvatar || finalAvatar.startsWith('data:')) {
        throw new Error('Avatar upload did not return a persistent URL');
      }
    }

    const profileRes = await fetch(`${API_BASE}/profile/user`, {
      method: 'PUT',
      headers: headers,
      body: JSON.stringify({ name, email, phone, bloodGroup: blood, location: city || '', age })
    });
    ...
    if (typeof INITIAL_DATA !== 'undefined' && INITIAL_DATA.familyMembers?.[0]) {
      Object.assign(INITIAL_DATA.familyMembers[0], { name, email, phone, bloodGroup: blood, age, avatar: finalAvatar });
    }
    ...
    console.error('[CuraAssist] Profile save failed:', err);
    alert(`Unable to save profile changes: ${err.message || err}`);
  }
}
